...
/// <reference path="phaser.js" />
/// <reference path="main.js" />
Card = function (point, group, elementNum, frameNum) {
    this.position = point;
    var cardImg = game.add.sprite(this.position.x, this.position.y, 'elementalBGs', elementNum, group);
    cardImg.anchor.setTo(0.5, 0.5);

    var buffer = 3;
    var frameImage = game.add.sprite(-cardImg.width / 2, -cardImg.height / 2, 'cardFrameSheet', frameNum)
    cardImg.anchor.setTo(0.5, 0.5);
    ...
    var character = game.add.sprite(-cardImg.width / 2, -cardImg.height / 2 - (Math.round(32 * scale)), 'wolf')
